    ...
    for epoch_i in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {"input_ids": batch[0].to(args.device),
                      ...
                      "labels": batch[3].to(args.device) if len(batch) <= 5 else batch[-1]} # add hard-label scheme
    ...
    for epoch_i in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {"input_ids": batch[0].to(args.device),
                      ...
                      "src_probs": batch[5]} # activate the KD loss
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = read_examples_from_file(args.data_dir, mode, args.num_sent)
...
def main():
    ...
    parser.add_argument("--tgt_model", type=str, default=None, help="for intitializing target model")
    parser.add_argument("--fix_position_emb", action="store_true", help="wheather to fix position embeddings")
    ...
    parser.add_argument("--joint_train", action="store_true", help="Enable join training?")
    ...
    if args.do_train:
        ...
        if args.fix_position_emb:
           setattr(config, "fix_position_emb", True)
        else:
            setattr(config, "fix_position_emb", False)
        tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lower_case=args.do_lower_case)
        model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool(".ckpt" in args.model_name_or_path), config=config, kernel_size=args.kernel_size)
        ...
        if args.do_KD:
            ...
            if args.tgt_model is not None:
                config = config_class.from_pretrained(args.tgt_model, num_labels=num_labels)
                model = model_class.from_pretrained(args.tgt_model, from_tf=bool(".ckpt" in args.tgt_model), config=config)
                model.to(args.device)
            # Train!
            if args.hard_label:
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_predictions_file, "w", encoding='utf-8') as writer:
            # change utf-8 to latin-1 for es
